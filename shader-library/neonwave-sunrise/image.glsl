...
// Replaces the original fract(sin(...)) hashes, whose precision collapses at the
// large, time-growing coordinates this shader feeds in (ro.z = TIME*0.25),
// causing vertical seams in the waves on some GPUs/backends.
float hash(float p) {
  p = fract(p * .1031);
  p *= p + 33.33;
  p *= p + p;
...
// "Hash without Sine" — Dave Hoskins. Stable 2->2 hash (no sin precision loss).
vec2 hash2(vec2 p) {
...
vec4 moon(vec3 ro, vec3 rd) {
  ...
  for (int i = -limit; i <= limit; ++i) {
    ...
    // max(msf,0): msf = sin(...) can be negative on the sphere's far side, and
    // pow(negative, non-integer) is NaN — which the DX12/HLSL backend propagates
    // (Vulkan masked it), showing as black dots in the FFT visualizer. Clamping to
    // 0 makes the term vanish there, which is the intended behaviour.
    fcol += mcol1*0.5*tanh_approx(0.0025/max(d0, 0.0))*imfre*pow(max(msf, 0.0), mix(100.0, 10.0, fft));
