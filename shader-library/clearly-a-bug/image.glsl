...
// mat2-from-vec4 spelled out column-major: naga's GLSL frontend mis-compiles the
// single-vec4 matrix constructor `mat2(vec4)` into a malformed Compose.
mat2 m2(vec4 v) { return mat2(v.x, v.y, v.z, v.w); }
...
void mainImage(out vec4 O, vec2 C) {
  ...
  for(
    ...
    , p.xy *= m2(cos(2.+O.z+vec4(0,11,33,0)))
