...
#define P(z) (vec3(cos((z)*.011)*16.+cos((z) * .012)  *24., \
                   cos((z)*.01)*4., (z)))
// mat2-from-vec4 fills column-major; spelled out so naga's GLSL frontend builds
// valid vec2 columns instead of a malformed Compose.
#define R(a) mat2(cos((a)+vec4(0,33,11,0)).x,cos((a)+vec4(0,33,11,0)).y,cos((a)+vec4(0,33,11,0)).z,cos((a)+vec4(0,33,11,0)).w)
#define N normalize
