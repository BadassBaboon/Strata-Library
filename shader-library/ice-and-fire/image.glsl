...
vec2 randCircle(vec3 p) {
    ...
    // Strata fix: as a long-running wallpaper, iTime (and thus this integer time
    // index p.z) grows unbounded; hash23 multiplies it by ~440 before fract(), so
    // fp32 precision decays over hours and the spline control points quantize ->
    // jerky motion. Wrapping the time index to a fixed period keeps the hash input
    // small (precise forever). Catmull-Rom stays C1-continuous through the wrapped
    // sequence, so the motion is seamless and just loops every 256 steps (~28 min).
    p.z = mod(p.z, 256.0);
