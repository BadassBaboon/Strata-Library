...
float mod1(inout float p, float size) {
  ...
  float res = mod(p + halfsize, size) - halfsize;
  p = res;
  return c;
}
...
vec3 color(vec3 ww, vec3 uu, vec3 vv, vec3 ro, vec2 p) {
  ...
  for(float i = 0.0; i < mm; ++i) {
    ...
    vec3 tcp = ro + tc.y*rd;
    float tcp2_x = tcp.z;
    float tcp2_y = atan(tcp.y, tcp.x);
  
    float zz = mix(0.025, 0.05, sqrt(h1))*rep/sz;
    float tnpy = mod1(tcp2_y, slices);
    float fo = smoothstep(0.5*slices, 0.25*slices, abs(tcp2_y));
    tcp2_x += -h2*TIME;
    tcp2_y *= tr*PI/3.0;

    tcp2_x /= zz;
    tcp2_y /= zz;
    vec2 tcp2 = vec2(tcp2_x, tcp2_y);
...
vec3 effect(vec2 p, vec2 pp) {
  ...
  {
    vec2 rxz = dro.xz;
    rxz *= ROT(0.2*sin(0.05*tm));
    dro.xz = rxz;
  }
  {
    vec2 ryz = dro.yz;
    ryz *= ROT(0.2*sin(0.05*tm*sqrt(0.5)));
    dro.yz = ryz;
  }
